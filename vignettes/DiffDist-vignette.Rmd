...
`DiffDist` currently takes a count matrix with samples in rows and genes in columns, the opposite of the usual convention. This will be fixed in later updates, but for now count matrices need to be transposed relative to the usual format.
```{r}
...
res <- ln_hmm_adapt_3_chains(counts=t(dat@count.matrix), 
...
p.mean.lfc1 <- apply(mean.diff, 2, hpd.pval, m=1) # Tests for differential expression at minimum LFC 1
p.disp.lfc1 <- apply(disp.diff, 2, hpd.pval, m=1) # Tests for differential dispersion at minimum LFC 1
```
...
# Genes with a posterior probability > threshold are declared as differentially distributed.
```
